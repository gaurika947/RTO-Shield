...
The transaction detail view evaluates feature mutations through the client deterministic fallback. It calculates baseline and modified predictions for phone verification, prepaid state, address completion, and network changes. Because the active artifact is not present in this checkout, these results must be treated as fallback-model simulations, not production-model explanations.
...
`ml/train.py` trains a scikit-learn `GradientBoostingClassifier` when the Python environment supports it and writes `ml/models/rto_model.joblib`, `model_trees.json`, and `model_meta.json`. The current checkout does not contain `rto_model.joblib`. Therefore `ml/predict.py` loads the artifact only when it exists; otherwise it returns `modelSource: deterministic_fallback` using its documented approximation.
...
The repository contains a synthetic/demo dataset generated by `ml/generate_data.py`:
...
The held-out test set is read by `ml/evaluate.py`. It is not used for training. The current runtime evaluation is explicitly marked as deterministic fallback-derived because the artifact is absent.
...
Training reads the existing split and writes model metadata plus artifacts when the dependency environment supports it. Evaluation reads `ml/data/test.csv`. Prediction accepts a JSON argument or uses its built-in sample. Running training changes model artifacts and should be an intentional model lifecycle action, not a demo startup step.
...
- The primary `rto_model.joblib` artifact is absent from this checkout, so runtime predictions use the labeled deterministic fallback.
